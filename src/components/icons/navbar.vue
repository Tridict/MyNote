<template>
  <template v-if="name === 'back'">
    <svg width="1.2rem" height="1.2rem" viewBox="0 0 24 24" name="mdi-chevron-left">
      <path
  ...
  <template v-if="name === 'showMore'">
    <svg
      width="1em"
      height="1em"
      viewBox="0 0 512 512"
      name="fa-solid:ellipsis-h"
    >
      <path
  ...
  <template v-if="name === 'showPreview'">
    <svg
      width="1.2rem"
      height="1.2rem"
      viewBox="0 0 24 24"
  ...
  <template v-if="name === 'sync'">
    <svg width="1.2rem" height="1.2rem" viewBox="0 0 24 24" name="mdi-cloud-sync-outline"><path d="M19 12v1.5c2.21 0 4 1.79 4 4c0 .82-.25 1.58-.67 2.21l-1.09-1.09c.17-.34.26-.72.26-1.12A2.5 2.5 0 0 0 19 15v1.5l-2.25-2.25l-.03-.03c.06-.05.13-.09 2.28-2.22m0 11v-1.5c-2.21 0-4-1.79-4-4c0-.82.25-1.58.67-2.21l1.09 1.09c-.17.34-.26.72-.26 1.12A2.5 2.5 0 0 0 19 20v-1.5l2.25 2.25l.03.03c-.06.05-.13.09-2.28 2.22m-5.97-5H6c-2.21 0-4-1.79-4-4s1.79-4 4-4h.71C7.37 7.69 9.5 6 12 6c3 0 5.4 2.37 5.5 5.32c.62-.21 1.3-.32 2-.32c1.28 0 2.47.38 3.5 1c-.87-1.1-2.16-1.86-3.65-1.97A7.49 7.49 0 0 0 12 4C9.11 4 6.6 5.64 5.35 8.03A6.004 6.004 0 0 0 6 20h7.5c-.26-.62-.42-1.3-.47-2z" fill="currentColor"></path></svg>
  </template>
  <template v-if="name === 'add'">
    <svg width="1.2rem" height="1.2rem" viewBox="0 0 24 24" name="ic-round-add"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z" fill="currentColor"></path></svg>
  </template>
</template>
